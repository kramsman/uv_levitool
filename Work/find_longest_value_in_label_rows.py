...
def max_label_lengths(*, used_field: str = '{priority}', label_docx = None, initial_attachment_dir = None) -> None:
    ...
    if nan_found_in_all_rows:
        ...
        df_non_nan = df[(df[keys] != 'nan').all(axis="columns")]  # note all columns taken
        if len(df_non_nan) != 0:
    ...
    nan_found_in_select_rows = (df[keys].loc[df[used_field].str.strip() != 'nan'] == 'nan')\
        .any(axis="columns").any(axis="rows")
    df_nan = df[keys][(df[used_field].str.strip() != 'nan') & (df[keys] == 'nan').any(axis="columns")]
    if nan_found_in_select_rows:
        ...
        pyautobek.alert(f"'Nan' (blank) found in {len(df_nan)} rows at least one key with non-blank '{used_field}' to be substituted so analysis "
